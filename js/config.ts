// Frequency in milliseconds to update the data
export const REFRESH_INTERVAL = 5 * 60 * 1000; // 5 minutes
export const FIRE_HOURS_REFRESH_INTERVAL = 45 * 1000; // 30 seconds
export const HOT_HOURS_REFRESH_INTERVAL = 90 * 1000; // 1.5 minutes
export const FRESH_HOURS_REFRESH_INTERVAL = 5 * 60 * 1000; // 5 minutes
export const DEAD_HOURS_REFRESH_INTERVAL = 15 * 60 * 1000; // 15 minute
export const CATCHUP_REFRESH_INTERVAL = 5 * 1000; // How long to wait after page load to catch up on stale records

export const FIRE_HOURS = 0.5;
export const HOT_HOURS = 2;
export const FRESH_HOURS = 16;
export const DEAD_HOURS = 5 * 24;

// Local storage keys
export const LEGACY_KEY_TEMPLATE_IDS = 'templateIDs';
export const KEY_TEMPLATE_IDS = 'template-ids';
export const KEY_WALLET = 'wallet';
export const KEY_COLUMN_OPTIONS = 'column-options';
